...
def test_get_items(dset: Dataset):
    assert len(dset.rows) == 1961726


def test_get_items_to_list(dset, fields):
    items = dset.rows
    first = items[0]
    l = first.to_list()
...
def test_get_items_to_dict(dset, fields):
    items = dset.rows
    first = items[0]
    d = first.to_dict()
...
def test_get_items_to_item_dict(dset, fields):
    items = dset.rows
    first = items[0]
    item_d = first.to_item_dict()
    expected_keys = list({f[0]: None for f in fields}.keys())
    assert list(item_d.keys()) == expected_keys
    assert None not in list(item_d.values())


def test_add_drop_fields(benchmark, dset: Dataset):
    @benchmark
    def _():
...
def test_filter_prefixes(benchmark, dset: Dataset):
    @benchmark
    def _():
        dset.filter_fields(lambda field: not ('pick_stats/' in field))
        assert len(dset.fields()) == 24


def test_copy_fields(benchmark, dset: Dataset):
    @benchmark
    def _():
        dset.copy_fields(['ctf/type'], ['bar'], ['O'])
        items = dset.rows
...
def test_append(benchmark, big_dset: Dataset):
    @benchmark
    def _():
        new_dset = big_dset.copy()
        new_dset.reassign_uids()
...
def test_append_union(benchmark, big_dset: Dataset):
    ...
    def _():
        other = big_dset.copy()
        other.append(big_dset, take_union=True)
...
def test_append_many(benchmark, big_dset, dset: Dataset):
    ...
    def _():
        ...
        other = big_dset.copy()
        other.reassign_uids()
        others = [dset, empty, other]

        new_dset = Dataset()
        new_dset.append_many(others)
        assert len(new_dset) == len(big_dset) * 2
...
def test_append_many_union(benchmark, big_dset, dset: Dataset):
    ...
    def _():
        ...
        other = dset.copy()
        other.reassign_uids()
        others = [dset, dset, empty, other]

        new_dset = Dataset()
        new_dset.append_many(others, take_union=True)
        assert len(new_dset) == len(big_dset) * 2
...
def test_append_many_union_repeat_allowed(benchmark, big_dset, dset: Dataset):
    ...
    def _():
        ...
        new_dset = Dataset()
        new_dset.append_many(others, take_union=True, repeat_allowed=True)
...
def test_append_many_simple(benchmark, big_dset, dset: Dataset):
    ...
    def _():
        ...
        new_dset = Dataset()
        new_dset.append_many_simple(others)
        assert len(new_dset) == len(big_dset) * 2
...
def test_append_many_simple_interlace(benchmark, big_dset, dset: Dataset):
    ...
    def _():
        ...
        new_dset = Dataset()
        new_dset.append_many_simple(others, interlace=True)
        assert len(new_dset) == len(big_dset) * 2
...
def test_append_replace(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other = Dataset(size=10000, fields=dset.dtype_descr())
        new_dset = dset.append_replace(other)
...
def test_append_replace_unique(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other = Dataset(size=10000, fields=dset.dtype_descr())
        new_dset = dset.append_replace(other, assume_unique=True)
        assert len(new_dset) == len(big_dset) + len(other)
        assert new_dset['uid'][-1] == other['uid'][-1]
...
def test_append_replace_empty(benchmark, big_dset, dset: Dataset):
    @benchmark
    def _():
        other = Dataset(fields=dset.dtype_descr())
        new_dset = dset.append_replace(other)
...
def test_append_replace_empty_query(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other = Dataset(fields=dset.dtype_descr())
        dset.append_replace(other, {'location/micrograph_uid': 6655121610611186569})
...
def test_append_replace_query_unique(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other = Dataset(size=10000, fields=dset.dtype_descr())
        dset.append_replace(other, {'location/micrograph_uid': 6655121610611186569}, assume_unique=True)
        assert len(dset) == len(big_dset) + len(other) - 1191
...
def test_append_replace_many(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other1 = Dataset(size=5000, fields=dset.dtype_descr())
        other2 = Dataset(size=5000, fields=dset.dtype_descr())
        new_dset = dset.append_replace_many([other1, other2])
...
def test_append_replace_many_unique(benchmark, big_dset: Dataset):
    ...
    def _():
        ...
        other1 = Dataset(size=5000, fields=dset.dtype_descr())
        other2 = Dataset(size=5000, fields=dset.dtype_descr())
        new_dset = dset.append_replace_many([other1, other2], assume_unique=True)
        assert len(new_dset) == len(big_dset) + len(other1) + len(other2)
        assert new_dset['uid'][-1] == other2['uid'][-1]
...
def test_append_replace_many_query(benchmark, big_dset: Dataset):
    ...
    def _():
        ...
        other1 = Dataset(size=5000, fields=dset.dtype_descr())
        other2 = Dataset(size=5000, fields=dset.dtype_descr())
        dset.append_replace_many([other1, other2],
            query={'location/micrograph_uid': [2539634023577218663, 6655121610611186569]},
            assume_unique=True)
...
def test_innerjoin(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        other = dset.range(500000, 1500000)
...
def test_innerjoin_many(benchmark, dset: Dataset):
    @benchmark
    def _():
        other1 = dset.range(500000, 1250000)
        other2 = dset.range(750000, 1500000)
        new_dset = Dataset()
...
def test_filter(benchmark, big_dset: Dataset):
    ...
    def _():
        dset = big_dset.copy()
        dset.filter([i for i in range(1500000) if i % 2 == 0])  # Even entries up to 1.5 million
...
def test_subset_idxs(benchmark, big_dset, dset: Dataset):
    @benchmark
    def _():
        new_dset = dset.subset([i for i in range(1500000) if i % 2 == 0])  # Even entries up to 1.5 million
...
def test_subset_simple_query_3(benchmark, dset: Dataset):
    ...
    def _():
        new_dset = dset.query({
            'uid': dset['uid'][1000:2000],
            'location/micrograph_path': 'J3/imported/18jam15a_0008_ali_DW.mrc'})
...
def test_subset_split_by(benchmark, dset: Dataset):
    @benchmark
    def _():
        dsets = dset.split('location/micrograph_uid')
...
def test_streaming_bytes(benchmark, dset: Dataset):
    ...
    def _():
        ...
        assert int(n.frombuffer(stream.read(4), dtype=n.uint32)[0]) == \
            len(json.dumps(dset.dtype_descr(), separators=(',', ':')).encode()) + 1  # not sure why + 1??
